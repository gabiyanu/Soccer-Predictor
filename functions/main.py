"""
Soccer Predictor — Firebase Cloud Function  (v2.0)
==================================================
Replaces Google App Engine (app.yaml) with Firebase Hosting + Cloud Functions.

Architecture
────────────
  Firebase Hosting  →  serves  web/index.html  (static)
  Firebase Hosting  →  rewrites  /api/**  →  Cloud Function `api`
  Cloud Function    →  Flask app handles all /api/* routes
  Firestore         →  prediction cache (replaces Flask-Caching)
  Gemini 1.5 Flash  →  AI match narratives  (optional)
  Google Sheets     →  live team-data management  (optional)
...
Environment Variables  (Firebase Console → Functions → Runtime Config)
───────────────────────────────────────────────────────────────────────
  GEMINI_API_KEY      Google Gemini API key for AI narratives
  GOOGLE_SHEETS_ID    Spreadsheet ID for live team-data management
  GOOGLE_SHEETS_JSON  Service-account credentials JSON string for Sheets
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# FIRESTORE CACHE  (replaces Flask-Caching)
# ══════════════════════════════════════════════════════════════════════════════
...
def api(req: https_fn.Request) -> https_fn.Response:
    ...
    Wraps the Flask app — handles all /api/* routes.
